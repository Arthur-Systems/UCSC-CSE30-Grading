...
def get_grades(course_id, headers, assignment_name, csv_file):
    ...
    for student in students:
        ...
        csv_grade = float(student_grades.get((first_name, last_name))) if (first_name, last_name) in student_grades else 0
        ...
        if submission and 'grade' in submission:
            ...
            if csv_grade > current_grade or current_grade == 0:
                if csv_grade * 0.8 > current_grade:
                    apply_late_penalty(course_id, assignment['id'], student['id'], headers, csv_grade)
                    print(f"Current Grade: {current_grade}, New Grade: {csv_grade}. Applied late penalty. Updated grade for student {student['sortable_name']}")
                elif current_grade == 0:
                    apply_late_penalty(course_id, assignment['id'], student['id'], headers, csv_grade)
                else:
                    print(f"Grade for student {student['sortable_name']} is lower than 80% of the original grade")
            else:
                pass
        else:
